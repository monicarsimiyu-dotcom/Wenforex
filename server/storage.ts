...
export interface IStorage {
  ...
  createTrade(trade: InsertTrade & { userId: string, accountType: string }): Promise<Trade>;
  ...
  createTransaction(tx: InsertTransaction & { userId: string, accountType: string }): Promise<Transaction>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateUserAccountType(userId: string, type: string): Promise<User> {
    const [updated] = await db.update(users).set({ activeAccountType: type }).where(eq(users.id, userId)).returning();
    return updated;
  }

  async getWallet(userId: string, accountType: string): Promise<Wallet | undefined> {
    const [wallet] = await db.select().from(wallets).where(and(eq(wallets.userId, userId), eq(wallets.accountType, accountType)));
    return wallet;
  }
  ...
  async updateBalance(userId: string, accountType: string, amount: number): Promise<Wallet> {
    ...
    const [updated] = await db.update(wallets).set({ balance: String(newBalance) }).where(and(eq(wallets.userId, userId), eq(wallets.accountType, accountType))).returning();
    return updated;
  }

  async createTrade(trade: InsertTrade & { userId: string, accountType: string }): Promise<Trade> {
    const [newTrade] = await db.insert(trades).values(trade).returning();
    return newTrade;
  }

  async getTrades(userId: string, accountType: string): Promise<Trade[]> {
    return db.select().from(trades).where(and(eq(trades.userId, userId), eq(trades.accountType, accountType))).orderBy(desc(trades.createdAt));
  }

  async updateTradeStatus(id: number, status: string, exitPrice: string, payout: string, closedAt: Date): Promise<Trade> {
    const [updated] = await db.update(trades).set({ status, exitPrice, payout, closedAt }).where(eq(trades.id, id)).returning();
    return updated;
  }

  async createTransaction(tx: InsertTransaction & { userId: string, accountType: string }): Promise<Transaction> {
  ...
  async getTransactions(userId: string, accountType: string): Promise<Transaction[]> {
    return db.select().from(transactions).where(and(eq(transactions.userId, userId), eq(transactions.accountType, accountType))).orderBy(desc(transactions.createdAt));
  }

  async updateTransactionStatus(reference: string, status: string): Promise<Transaction> {
    const [updated] = await db.update(transactions).set({ status }).where(eq(transactions.reference, reference)).returning();
    return updated;
  }
}
